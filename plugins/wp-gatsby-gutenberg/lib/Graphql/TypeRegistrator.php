...
if (!defined('WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_POSTS_FIELD_NAME')) {
	define(
		'WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_POSTS_FIELD_NAME',
		'gutenbergPosts'
	);
}
...
if (!defined('WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_POST_CONNECTION_NAME')) {
	define(
		'WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_POST_CONNECTION_NAME',
		'GutenbergPostConnection'
	);
}

if (
	!defined(
		'WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_NAMES_FIELD_NAME'
	)
) {
	define(
		'WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_NAMES_FIELD_NAME',
...
if (
	!defined(
		'WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_RENDER_FIELD_NAME'
	)
) {
	define(
		'WP_GATSBY_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_RENDER_FIELD_NAME',
		'gutenbergRenderDynamicBlock'
	);
}
...
class TypeRegistrator
{
	...
	{
		return array_filter(\WPGraphQL::get_allowed_post_types(), function (
			$post_type
		) {
	...
	public function __construct()
	{
		...
		add_filter('graphql_wp_object_type_config', function ($config) use (
			$gutenberg_post_type_interface_config
		) {
			if (
				in_array(
					...
					array_map(
						'strtolower',
						$this->get_gutenberg_graphql_types()
					)
				)
			) {
						...
						'resolve' => function ($source, $args) {
							...
							if (
								...
								return $server_block_type->render(
									json_decode($args['attributesJSON'], true)
								);
							}
