...
if (!defined('GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_NAMES_FIELD_NAME')) {
  define('GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_NAMES_FIELD_NAME', 'gutenbergDynamicBlockNames');
}

if (!defined('GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_RENDER_FIELD_NAME')) {
          ...
          foreach ($gutenberg_post_type_interface_config['fields'] as $key => $value) {
            if (substr($key, 0, strlen('gutenberg')) === 'gutenberg') {
              $fields[$key] = $this->type_registry
                ->get_type(GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_POST_TYPE_INTERFACE_NAME)
                ->getField($key);
            }
          }
    ...
    add_action(
      ...
      function ($type_registry) use ($gutenberg_post_type_interface_config) {
        ...
        register_graphql_interface_type(
          GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_POST_TYPE_INTERFACE_NAME,
          $gutenberg_post_type_interface_config
        );
        register_graphql_field(
          'RootQuery',
          GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_NAMES_FIELD_NAME,
          [
        ...
        register_graphql_field(
          'RootQuery',
          GATSBY_WORDPRESS_GUTENBERG_GRAPHQL_GUTENBERG_DYNAMIC_BLOCK_RENDER_FIELD_NAME,
          [
            'type' => 'String',
            'args' => [
